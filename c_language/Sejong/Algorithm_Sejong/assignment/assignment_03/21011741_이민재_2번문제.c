...
int findElement(int k) // 검색
{
	...
	if (isExternal(w))
	// 찾은 노드가 external이면 -1을 반환한다
	{
		return -1;
	}
	else
	{
		// 찾은 노드가 internal이면 key값을 반환한다.
		return w->key;
	}
}
...
void insertItem(int k)
{
	...
	{
		w->key = k;		   // 찾은 노드가 external이면 expandExternal을 호출한다.
		expandExternal(w); // 찾은 노드의 key값을 k로 설정한다.
...
void printTree(node *w) // 트리 출력
{
	...
	{
		printf(" %d", w->key); // ₩->lChild로 재귀호출
		printTree(w->lChild); // w의 key값 출력
...
void searchAndFixAfterInsertion(node *w)
{
	...
	// 불균형이 발생한 노드를 z라고 하고, z의 부모를 y라고 하고, y의 부모를 x라고 한다.
	if (isBalanced(z))
	{
		return;
	}
